...
<div class="maindashatten my-5">
    <div class="modal-content">

        <div class="card-body">
            <form action="{{ route('employe-leave.store') }}" method="POST" enctype='multipart/form-data'>
                @csrf
                <div class="row">
                    <div class="row gap-3 flex-wrap ">
                        <select class="col bgthemelight rounded-5 border py-3 px-3 mx-auto secondclr fw-600" id="leave_type_id" name="leave_type_id" aria-label="leave_type_id" aria-describedby="leave_type_id">
                            ...
                            @foreach($leaveType as $attend)
                            <option value="{{ $attend->id }}" {{ (collect(old('leave_type_id'))->contains($attend->id)) ? 'selected':'' }}>
                                {{ $attend->leave_type }}
                            </option>
                            @endforeach
                        </select>
                        ...
                        <input type="file" onchange="previewImage('.filImageInput', '.diplayImage')" class="col bgthemelight rounded-5 border py-3 px-3 mx-auto secondclr fw-600 filImageInput" name="request_hard_copy" id="request_hard_copy">
                        @php
                        ...
                        @enderror
                        <div class="avatar">
                            <img src="{{ $imagUrl }}" class="rounded-circle diplayImage" style="width: 45px; height: 45px; margin-left: 53%;" id="wizardPicturePreview">
                        </div>
                    </div>

                    <div class="row gap-3 flex-wrap my-3">
                        <label for="request_start_date" class="form-label mt-2">From Date</label>
                        <input type="date" class="col bgthemelight rounded-5 border secondclr fw-600" placeholder="{{ __('Request Start Date') }}" name="request_start_date" id="request_start_date" value="{{ old('request_start_date') }}">
                        ...
                        <select type="Number" class="col bgthemelight rounded-5 border py-3 px-3 mx-auto secondclr fw-600" id="request_leave_type_out_id" name="request_leave_type_out_id" aria-label="request_leave_type_out_id" aria-describedby="request_leave_type_out_id">
                            ...
                            <option value="{{ $attend->id }}" {{ (collect(old('request_leave_type_out_id'))->contains($attend->id)) ? 'selected':'' }}>
                                {{ $attend->name }}
                            </option>
                            @endforeach
                    ...
                    <div class="row gap-3 flex-wrap my-3">
                        ...
                        <input type="date" class="col bgthemelight rounded-5 border secondclr fw-600" placeholder="{{ __('Request End Date') }}" name="request_end_date" id="request_end_date" value="{{ old('request_end_date') }}">
                        ...
                        <select type="Number" class="col bgthemelight rounded-5 border py-3 px-3 mx-auto secondclr fw-600" id="request_leave_type_in_id" name="request_leave_type_in_id" aria-label="request_leave_type_in_id" aria-describedby="request_leave_type_in_id">
                            ...
                            @foreach($Leave_list as $attend)
                            <option value="{{ $attend->id }}" {{ (collect(old('request_leave_type_in_id'))->contains($attend->id)) ? 'selected':'' }}>
                                {{ $attend->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="row gap-3 flex-wrap my-5">
                        <input type="text" class="col bgthemelight rounded-4 border py-3 px-3 mx-auto secondclr fw-600" placeholder="Request Remark" name="request_remark" id="request_remark" value="{{ old('request_remark') }}">
                        @error('request_remark')
                        <p class="text-danger text-xs mt-2">{{ $message }}</p>
                        @enderror

                        <input type="number" name="request_day" id="sum"
                            class="col bgthemelight rounded-4 border py-3 px-3 mx-auto secondclr fw-600" 
                            readonly value="{{ old('request_day') }}" />
                        @error('request_day')
                        <p class="text-danger text-xs mt-2">{{ $message }}</p>
                        @enderror
                    ...
                    <button type="submit"
                        class="btn btn-primary w-50 p-2 subhead rounded-5 bgtheme mb-0 button text-light mx-auto submitbtn">
                        Submit</button>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#request_leave_type_out_id, #request_leave_type_in_id,#request_start_date,#request_end_date').change(
            function() {
