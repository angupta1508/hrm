...
<div class="accordion card filter_card mb-4" id="accordionFilter">
        <div class="accordion-item mb-3">
            ...
            <div id="collapseFilter" class="accordion-collapse collapse @if (!empty($filter)) show @endif"
                ...
                <div class="accordion-body card-body p-3 text-sm opacity-8">
                    <form action="{{ route('admin.attendence.attendance.attendanceLog') }}" method="GET">
                        @csrf
                        <div class="border">
                            <div class="d-flex flex-row align-content-between flex-wrap">
                                <div class="p-2 flex-fill w-100">
                                    <input type="text" id="search" name="search" class="form-control"
                                        ...
                                        placeholder="{{ __('Search By User Name, Employee Code,Punch Type') }}">
                                </div>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body p-3">
                ...
                <table class="table align-items-center mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                @sortablelink('punch_type', __('Punch Type'))
                            </th>
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                @sortablelink('from_where', __('From Where'))
                            </th>
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                @sortablelink('punch_time', __('Punch Time'))
                            </th>
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                @sortablelink('image', __('Image'))
                            </th>
                    ...
                    <tbody>
                        @foreach ($attendance_log as $attend)
                            <tr>
                                ...
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ $attend->punch_type }}</p>
                                </td>

                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ $attend->from_where }}</p>
                                </td>

                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ $attend->punch_time }}</p>
                                </td>

                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ $attend->image }}</p>
                                </td>
                                ...
                                <td class="text-end">
                                    <form action="{{ route('admin.attendence.attendance.destroy', $attend->id) }}" method="Post">
                                        @csrf
                                        @method('DELETE')
            ...
            {{ $attendance_log->appends($filter)->links('pagination::bootstrap-5') }}
        </div>
            </div>
