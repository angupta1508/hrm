        ...
        <div class="accordion-item mb-3">
            ...
            <div id="collapseFilter" class="accordion-collapse collapse @if (!empty($filter)) show @endif"
                ...
                <div class="accordion-body card-body p-3 text-sm opacity-8">
                    <form action="{{ route('admin.cms.pages.index') }}" method="GET">
                        @csrf
                        <div class="border">
                            <div class="d-flex flex-row align-content-between flex-wrap">
                                ...
                                <div class="p-2 flex-fill">
                                    <input type="text" id="search" name="search" class="form-control"
                                        ...
                                        placeholder="{{__('Search By Page Slug')}}">
                                </div>
                                <div class="p-2 flex-fill">
    ...
    <div class="card mb-4">
        <div class="card-header p-3">
            <div class="d-flex flex-row justify-content-between">
                <div>
                    <h5 class="mb-0">{{ __('All Pages') }}</h5>
                </div>
                <a href="{{ route('admin.cms.pages.create') }}" class="btn bg-gradient-primary btn-sm mb-0" type="button"><i
                        class="fa fa-plus" aria-hidden="true"></i> {{__('Add New pages')}}</a>
            </div>
        </div>
        <div class="card-body p-3">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                ...
                                @sortablelink('page_name', __('Page Name'))
                            </th>
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">

                                @sortablelink('page_slug', __('Page Slug'))
                            </th>
                    ...
                    <tbody>
                        @foreach ($pages as $page)
                            <tr>
                                ...
                                <td class="">
                                    <p class="text-xs font-weight-bold mb-0">{{ $page->page_slug }}</p>
                                </td>
                                <td class="text-center">
                                    <div class="form-check form-switch d-inline-block">
                                        <input class="form-check-input updateStatus" type="checkbox" role="switch"
                                            {{ $page->status ? 'checked' : '' }} data-id="{{ $page->id }}">
                                    </div>
                                </td>
                                ...
                                <td class="text-center">
                                    <form action="{{ route('admin.cms.pages.destroy', $page->id) }}" method="Post">
                                        <a class="btn btn-warning btn-sm text-white"
                                            href="{{ route('admin.cms.pages.edit', $page->id) }}"><i
            ...
            {{ $pages->appends($filter)->links('pagination::bootstrap-5') }}
        ...
        <script>
            $(document).ready(function() {
                $('.updateStatus').on('change', function() {
                    ...
                    //
                    $.ajax({
                        url: "{{ route('admin.cms.pages.changeStatus') }}",
                        type: 'post',
                        data: {
                            _token: _token,
                            id: id,
