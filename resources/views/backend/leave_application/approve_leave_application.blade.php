    ...
    <div class="col-md-6">
      <div class="card">
        <div class="card-body">
          ...
          <div class=""><strong class="text-dark">{{__('Leave Type')}} :-</strong>&nbsp;
          ...
          <div class="mt-2"><strong class="text-dark">{{__('From Date')}} :-</strong>&nbsp;
          ...
          <div class="mt-2"><strong class="text-dark">{{__('To Date')}} :-</strong>&nbsp;
          ...
          <div class="mt-2"><strong class="text-dark">{{__('Request Days')}} :-</strong>&nbsp;
            {{ !empty($leave_data->request_day) ? $leave_data->request_day : '' }}
          </div>


          <div class="mt-2"><strong class="text-dark">{{__('Request Hard Copy')}} :-</strong>&nbsp;
            <div class="mt-2">
              <p class="gallery">
                <a href="{{ url(config('constants.leave_request_hard_copy_image_path') . $leave_data->request_hard_copy) }}" data-fancybox="group" data-caption="This image has a caption 1">
                  <img src="{{ url(config('constants.leave_request_hard_copy_image_path') . $leave_data->request_hard_copy) }}" style="height: 50px; width: 50px;" alt="{{__('request_hard_copy')}}" title="request_hard_copy" />
                </a>
              </p>
            </div>
          </div>
    ...
    <div class="col-md-6 text-sm">
      <div class="card">
        <div class="card-body text-sm">
          ...
          <div class="mt-2"><strong class="text-dark">Admin Name :-</strong>&nbsp;
            {{ !empty($leave_data->approve_name) ? $leave_data->approve_name : '' }}
          </div>
      ...
      <div class="card">
        <div class="card-body">
          <label class="mt-2" style="font-size: 18px;font-weight: bold">
            {{__('Authorised Person Detail')}}
          </label>

          <div class="mt-2"><strong class="text-dark"> {{__('Name')}} :-</strong>&nbsp;
            {{ !empty($leave_data->authorise_name) ? $leave_data->authorise_name : '' }}
          </div>

          <div class="mt-2"><strong class="text-dark">{{__('Email')}} :-</strong>&nbsp;
            {{ !empty($leave_data->email) ? $leave_data->email : '' }}
          </div>

          <div class="mt-2"><strong class="text-dark">{{__('Mobile No')}} :-</strong>&nbsp;
    ...
    <div class="col-md-6">
      <div class="card">
        <div class="card-body text-sm">
          ...
          <div class="mt-2"><strong class="text-dark"> {{__('Employee Code')}} :-</strong>&nbsp;
            {{ !empty($leave_data->user_name) ? $leave_data->employee_code : '' }}
          </div>

          <div class="mt-2"><strong class="text-dark">{{__('Company Id')}} :-</strong>&nbsp;
            {{ !empty($leave_data->user_name) ? $leave_data->company_id : '' }}
          </div>

          <div class="mt-2"><strong class="text-dark">{{__('Machine Code')}} :-</strong>&nbsp;
    ...
    <div class="col-12">
      <div class="multisteps-form mb-5">
        <div class="col-md-12">
          <div class="card">
            <div class="card-body">
              <form method="post" action="{{ route('admin.leave.approve_leave_application.approveStatus', $leave_data) }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="mt-4" style="font-size: 13px;font-weight: bold">
                      <input type="radio" id="pending" name="status" value="0" {{ old('status', $leave_data->status) == 0 ? 'checked' : '' }}> <label for="pending">{{__('Pending')}}</label>
                      <input type="radio" id="approve" name="status" value="1" {{ old('status',  $leave_data->status) == 1 ? 'checked' : '' }}> <label for="approve">{{__('Approved')}}</label>
                      <input type="radio" id="cancel" name="status" value="2" {{ old('status', $leave_data->status) == 2 ? 'checked' : '' }}> <label for="cancel"> {{__('Cancelled')}}</label>
                    </div>
                  </div>
                </div>
