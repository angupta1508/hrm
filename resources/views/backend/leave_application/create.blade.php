...
<div class="card card-default">
    ...
    <div class="card-body">
        <form action="{{ route('admin.leave.leaves.store') }}" method="POST" enctype='multipart/form-data'>
            @csrf
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    <div class="">
                        <select class="form-select  select2" id="leave_type_id" name="leave_type_id" aria-label="leave_type_id" aria-describedby="leave_type_id">
                            ...
                            <option value="{{$attend->id}}" {{ (collect(old('leave_type_id'))->contains($attend->id)) ? 'selected':'' }}>{{ $attend->leave_type }}</option>
                            @endforeach
                        </select>
                        @error('leave_type_id')
                        <p class="text-danger text-xs mt-2">{{ $message }}</p>
                ...
                <div class="col-md-6">
                    ...
                    <div class="">
                        <input type="date" class="form-control date" placeholder="{{ __('Request Start Date') }}" name="request_start_date" id="request_start_date" value="{{ old('request_start_date') }}">
                ...
                <div class="col-md-6">
                    ...
                    <div class="">
                        <select type="Number" class="form-select  select2" id="request_leave_type_out_id" name="request_leave_type_out_id" aria-label="request_leave_type_out_id" aria-describedby="request_leave_type_out_id">
                            <option value="">{{__('Please Select Leave Type Out')}}</option>
                            @foreach($LeaveOut as $attend)
                            <option value="{{$attend->id}}" {{ (collect(old('request_leave_type_out_id'))->contains($attend->id)) ? 'selected':'' }}>{{ $attend->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6">
                    ...
                    <div class="">
                        <input type="date" class="form-control date" placeholder="{{ __('Request End Date') }}" name="request_end_date" id="request_end_date" value="{{ old('request_end_date') }}">
                ...
                <div class="col-md-6">
                    ...
                    <div class="">
                        <select type="Number" class="form-select  select2" id="request_leave_type_in_id" name="request_leave_type_in_id" aria-label="request_leave_type_in_id" aria-describedby="request_leave_type_in_id">
                            <option value="">{{__('Please Select Leave Type')}}</option>
                            @foreach($Leave_list as $attend)
                            <option value="{{$attend->id}}" {{ (collect(old('request_leave_type_in_id'))->contains($attend->id)) ? 'selected':'' }}>{{ $attend->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6">
                    <label class="form-label mt-4" for="sum">{{ __('Request Days') }}</label>
                    <div class="">
                        <input type="number" name="request_day" id="sum" class="form-control sum" readonly value="{{ old('request_day') }}" />
                        @error('request_day')
                        <p class="text-danger text-xs mt-2">{{ $message }}</p>
                        @enderror
                    </div>
