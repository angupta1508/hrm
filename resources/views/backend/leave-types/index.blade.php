        ...
        <div class="accordion-item mb-3">
            ...
            <div id="collapseFilter" class="accordion-collapse collapse @if (!empty($filter)) show @endif"
                ...
                <div class="accordion-body card-body p-3 text-sm opacity-8">
                    <form action="{{ route('admin.leave.leave-types.index') }}" method="GET">
                        @csrf
                        <div class="border">
                            <div class="d-flex flex-row align-content-between flex-wrap">
                                <div class="p-2 flex-fill">
                                    <input type="text" id="leave_type" name="leave_type" class="form-control"
                                        value="{{ !empty($filter['leave_type']) ? $filter['leave_type'] : '' }}" autocomplete="off"
                                        placeholder="{{ __('Search By Leave Type') }}">
                                </div>
                                <div class="p-2 flex-fill">
    ...
    <div class="card mb-4">
        <div class="card-header p-3">
            <div class="d-flex flex-row justify-content-between">
                <div>
                    <h5 class="mb-0">{{ __('Leave Types') }}</h5>
                </div>
                <a href="{{ route('admin.leave.leave-types.create') }}" class="btn bg-gradient-primary btn-sm mb-0">
        ...
        <div class="card-body p-3">
                ...
                <table class="table align-items-center mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                @sortablelink('leave_type', __('Leave Type'))
                            </th>
                        ...
                        @foreach ($leave_types as $leave_type)
                            <tr>
                                ...
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ wordlimit($leave_type->leave_type) }}
                                    </p>
                                </td>
                                ...
                                <td class="text-center">
                                    <div class="form-check form-switch d-inline-block">
                                        <input class="form-check-input updateStatus" type="checkbox" role="switch"
                                            {{ $leave_type->status ? 'checked' : '' }} data-id="{{ $leave_type->id }}">
                                    </div>
                                </td>
                                <td class="text-end">
                                    <form action="{{ route('admin.leave.leave-types.destroy', $leave_type->id) }}" method="Post">
                                        <a class="btn btn-warning btn-sm text-white" title="{{ __('Edit') }}"
                                            data-toggle="tooltip" href="{{ route('admin.leave.leave-types.edit', $leave_type->id) }}">
            ...
            {{ $leave_types->appends($filter)->links('pagination::bootstrap-5') }}
        </div>
    </div>
@endsection
    ...
    <script>
        $(document).ready(function() {
            $('.updateStatus').on('change', function() {
                ...
                //
                $.ajax({
                    url: "{{ route('admin.leave.leave-type.changeStatus') }}",
                    type: 'post',
                    data: {
                        _token : _token,
                        id: id,
