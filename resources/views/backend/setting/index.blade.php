                ...
                <div class="card mb-4 mx-4">
                    <div class="card-header pb-0">
                        <div class="d-flex flex-row justify-content-between">
                            <div>
                                <h5 class="mb-0">{{ __('All') }} {{__('Settings')}}</h5>
                            </div>
                        </div>
                    </div>
                    <div class="card-body pb-0">
                        @if (!empty($no_change_msg))
                            <div class="text-center bg-primary p-2">
                                <h6 class="text-white">{{ getListTranslate($no_change_msg) }}</h6>
                        ...
                        <div class="table-responsive p-0">
                            <form action="{{ route('admin.setting.updateSetting') }}" method="POST"
                                        ...
                                        @foreach ($settings as $setting)
                                            <tr>
                                                <td class="text-center" style="width:50px;">
                                                    ...
                                                    {{ getListTranslate($setting->setting_label) }}
                                                    </p>
                                                </td>
                                                <td class="text-left">
                                                        ...
                                                        <div class="row">
                                                            <div class="col-8">
                                                                <input type="file" class="form-control"
                                                                    ...
                                                                    value="{{ old($setting->setting_name, $setting->setting_value) }}">
                                                            </div>
                                                            <div class="col-4">
                                                    ...
                                                    @elseif($setting->input_type == 'slider')
                                                        <div class="form-check form-switch d-inline-block">
                                                            <input class="form-check-input updateStatus" type="checkbox"
                                                                ...
                                                                name="setting[{{ $setting->setting_name }}]"
                                                                value="1"
                                                                {{ old($setting->setting_name, $setting->setting_value) == 1 ? 'checked' : '' }}>
                                                        </div>
                                            @else
                                                <input type="text" class="form-control"
                                                    placeholder="{{ getListTranslate($setting->setting_name) }}"
                                                    name="setting[{{ $setting->setting_name }}]"
                                                    id="setting_{{ $setting->setting_name }}"
                                                    value="{{ old($setting->setting_name, $setting->setting_value) }}">
                                        @endif
                                ...
                                <div class="d-flex justify-content-end mb-4">
                                    <button type="submit" name="button" class="btn bg-gradient-primary m-0 ms-2">{{__('Save Setting')}}</button>
                                </div>
                            </form>
                        </div>
                    </div>
