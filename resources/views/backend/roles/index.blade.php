        ...
        <div class="accordion-item mb-3">
            ...
            <div id="collapseFilter" class="accordion-collapse collapse @if (!empty($filter)) show @endif"
                ...
                <div class="accordion-body card-body p-3 text-sm opacity-8">
                    <form
                        action="{{ Request::routeIs('admin.' . $routeSlug . '.index') ? route('admin.' . $routeSlug . '.index') : route('admin.roles.index') }}"
                        method="GET">
                        @csrf
                        <div class="border">
                        <!-- @foreach ($roles as $role)
                    @csrf
                    @if ($role->role_type == '' || $role->slug == config('constants.role_slug_superadmin-staff'))
                            <a href=" "
    ...
    <div class="card mb-4">
        <div class="card-header p-3">
            <div class="d-flex flex-row justify-content-between">
                <div>
                    <h5 class="mb-0">{{ __('All') }} {{ __('Roles') }}</h5>
                </div>
                @if (Config::get('auth_detail')['role_id']  ==  config('constants.superadmin_role_id')) 
                        <a href=""
                        class="btn bg-gradient-primary btn-sm mb-0" type="button">
                        <i class="fa fa-plus" aria-hidden="true"></i>
                        {{ __('Add New Role') }}
        ...
        <div class="card-body p-3">
                ...
                <table class="table align-items-center mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-left text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                @sortablelink('role_type', __('Role Type'))
                            </th>
                            <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                @sortablelink('created_at', __('Creation Date'))
                            </th>
                            <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                    ...
                    <tbody>
                        @foreach ($roles as $role)
                            <tr>
                                ...
                                <td class="text-left">
                                    <p class="text-xs font-weight-bold mb-0">{{ $role->role_type }}</p>
                                </td>
                                ...
                                <td class="text-center">
                                    <div class="form-check form-switch d-inline-block">
                                        <input class="form-check-input updateStatus" type="checkbox" role="switch"
                                            {{ $role->status ? 'checked' : '' }} data-id="{{ $role->id }}">
                                    </div>
                                </td>
                                <td class="text-end">
                                    <form
                                        action="{{ Request::routeIs('admin.' . $routeSlug . '.index') ? route('admin.' . $routeSlug . '.destroy', $role->id) : route('admin.roles.destroy', $role->id) }}"
                                        method="Post">
                                        @csrf
                                        @if ($role->role_type == 'Admin' || $role->slug == config('constants.role_slug_superadmin-staff'))
                                        ...
                                        <a class=" btn btn-warning btn-sm text-white"
                                            href="{{ Request::routeIs('admin.' . $routeSlug . '.index') ? route('admin.' . $routeSlug . '.edit', $role->id) : route('admin.roles.edit', $role->id) }}">
            ...
            {{ $roles->appends($filter)->links('pagination::bootstrap-5') }}
        </div>
    </div>
    @push('dashboard')
        <script>
