        ...
        <div class="accordion-item mb-3">
            ...
            <div id="collapseFilter" class="accordion-collapse collapse @if (!empty($filter)) show @endif"
                ...
                <div class="accordion-body card-body p-3 text-sm opacity-8">
                    <form action="{{ route('admin.sms-template.index') }}" method="GET">
                        @csrf
                        <div class="border">
                            <div class="d-flex flex-row align-content-between flex-wrap">
                                ...
                                <div class="p-2 flex-fill">
                                    <input type="text" id="search" name="search" class="form-control"
                                        ...
                                        placeholder="{{__('Search By Sms Title')}}">
                                </div>
                                <div class="p-2 flex-fill">
    ...
    <div class="card mb-4">
        <div class="card-header p-3">
            <div class="d-flex flex-row justify-content-between">
                <div>
                    <h5 class="mb-0">{{ __('All') }} {{__('SMS Template')}}</h5>
                </div>
                <a href="{{ route('admin.sms-template.create') }}" class="btn bg-gradient-primary btn-sm mb-0"
                    type="button"><i class="fa fa-plus" aria-hidden="true"></i> {{__('Add New Template')}}</a>
            </div>
        </div>
        <div class="card-body p-3">
                    ...
                    <tbody>
                        @foreach ($templates as $temp)
                            <tr>
                                ...
                                <td class="">
                                    <p class="text-xs font-weight-bold mb-0">{{ $temp->title }}</p>
                                </td>
                                <td class="text-center">
                                    <div class="form-check form-switch d-inline-block">
                                        <input class="form-check-input updateStatus" type="checkbox" role="switch"
                                            {{ $temp->status ? 'checked' : '' }} data-id="{{ $temp->id }}">
                                    </div>
                                </td>

                                <td class="text-center">
                                    <form action="{{ route('admin.sms-template.destroy', $temp->id) }}" method="Post">
                                        <a class="btn btn-warning btn-sm text-white"
                                            href="{{ route('admin.sms-template.edit', $temp->id) }}"><i
                                                class="fas fa-edit"></i></a>
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm text-white" title="Delete"
                                            data-toggle="tooltip"><i class="cursor-pointer fas fa-trash"></i></button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
            ...
            {{ $templates->appends($filter)->links('pagination::bootstrap-5') }}
        </div>
    </div>
@endsection
@push('dashboard')
    <script>
        $(document).ready(function() {
            $('.updateStatus').on('change', function() {
                ...
                //
                $.ajax({
                    url: "{{ route('admin.sms-template.changeStatus') }}",
                    type: 'post',
                    data: {
                        _token: _token,
                        id: id,
