        ...
        <div class="accordion-item mb-3">
            ...
            <div id="collapseFilter" class="accordion-collapse collapse @if (!empty($filter)) show @endif"
                ...
                <div class="accordion-body card-body p-3 text-sm opacity-8">
                    <form action="{{ route('admin.administration.companies.index') }}" method="GET">
                        @csrf 
                        <div class="border">
    ...
    <div class="card mb-4">
        <div class="card-header p-3">
            <div class="d-flex flex-row justify-content-between">
                ...
                <a href="{{ route('admin.administration.companies.create') }}" class="btn bg-gradient-primary btn-sm mb-0">
        ...
        <div class="card-body p-3">
                        ...
                        @foreach ($companies as $company)
                            <tr>
                                ...
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ wordlimit($company->name) }}
                                    </p>
                                </td>
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ $company->created_at }}</p>
                                </td>
                                <td class="text-center">
                                    <div class="form-check form-switch d-inline-block">
                                        <input class="form-check-input updateStatus" type="checkbox" role="switch"
                                            {{ $company->status ? 'checked' : '' }} data-id="{{ $company->id }}">
                                    </div>
                                </td>
                                <td class="text-end">
                                    <form action="{{ route('admin.administration.companies.destroy', $company->id) }}" method="Post">
                                        <a class="btn btn-warning btn-sm text-white" title="{{ __('Edit') }}"
                                            data-toggle="tooltip" href="{{ route('admin.administration.companies.edit', $company->id) }}">
            ...
            {{ $companies->appends($filter)->links('pagination::bootstrap-5') }}
        </div>
    </div>
@endsection
    ...
    <script>
        $(document).ready(function() {
            $('.updateStatus').on('change', function() {
                ...
                //
                $.ajax({
                    url: "{{ route('admin.administration.companies.changeStatus') }}",
                    type: 'post',
                    data: {
                        _token: _token,
                        id: id,
