        ...
        <div class="accordion-item mb-3">
            ...
            <div id="collapseFilter" class="accordion-collapse collapse @if (!empty($filter)) show @endif"
                ...
                <div class="accordion-body card-body p-3 text-sm opacity-8">
                    <form action="{{ route('admin.email-template.index') }}" method="GET">
                        @csrf
                        <div class="border">
                            <div class="d-flex flex-row align-content-between flex-wrap">
                                ...
                                <div class="p-2 flex-fill">
                                    <input type="text" id="search" name="search" class="form-control"
                                        ...
                                        placeholder="{{__('Search By Email Title')}}">
                                </div>
                                <div class="p-2 flex-fill">
    ...
    <div class="card mb-4">
        <div class="card-header p-3">
            <div class="d-flex flex-row justify-content-between">
                <div>
                    <h5 class="mb-0">{{ __('All') }} {{ __('Template') }}</h5>
                </div>
                <a href="{{ route('admin.email-template.create') }}" class="btn bg-gradient-primary btn-sm mb-0"
                    type="button">
                    ...
                    <tbody>
                        @foreach ($templates as $temp)
                            <tr>
                                ...
                                <td class="text-center">
                                    <form action="{{ route('admin.email-template.destroy', $temp->id) }}" method="Post">
                                        <a class="btn btn-warning btn-sm text-white"
                                            href="{{ route('admin.email-template.edit', $temp->id) }}"><i
    ...
    <script>
        $(document).ready(function() {
            $('.updateStatus').on('change', function() {
                ...
                //
                $.ajax({
                    url: "{{ route('admin.email-template.changeStatus') }}",
                    type: 'post',
                    data: {
                        _token: _token,
                        id: id,
