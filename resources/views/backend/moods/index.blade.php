        ...
        <div class="accordion-item mb-3">
            ...
            <div id="collapseFilter" class="accordion-collapse collapse @if (!empty($filter)) show @endif"
                ...
                <div class="accordion-body card-body p-3 text-sm opacity-8">
                    <form action="{{ route('admin.moods.moods.index') }}" method="GET">
                        @csrf
                        <div class="border">
                            <div class="d-flex flex-row align-content-between flex-wrap">
                                <div class="p-2 flex-fill">
                                    {{ Form::select('user_id', ['' => __('Select Employee')] + $getemployees, isset($filter['user_id']) ? $filter['user_id'] : '', ['class' => 'form-select']) }}
                                </div>


                                <div class="p-2 flex-fill">
                                    {{ Form::select('mood_type', ['' => __('Select Mood')] + getListTranslate(config('constants.default_mood_type')), isset($filter['mood_type']) ? $filter['mood_type'] : '', ['class' => 'form-select']) }}
                                </div>
    ...
    <div class="card mb-4">
        {{-- <div class="card-header p-3">
            <div class="d-flex flex-row justify-content-between">
                <div>
                    <h5 class="mb-0">{{ __('Moods') }}</h5>
                </div>
                <a href="{{ route('admin.moods.moods.create') }}" class="btn bg-gradient-primary btn-sm mb-0">
                        ...
                        @foreach ($moods as $mood)
                            <tr>
                                ...
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ $mood->user_name }}
                                    </p>
                                </td>
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ $mood->mood_type }}
                                    </p>
                                </td>
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ $mood->remark }}
                                    </p>
                                </td>
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ $mood->created_at }}</p>
                                </td>
                                <td class="text-end">
    ...
    <script>
        $(document).ready(function() {
            $('.updateStatus').on('change', function() {
                ...
                //
                $.ajax({
                    url: "{{ route('admin.moods.moods.changeStatus') }}",
                    type: 'post',
                    data: {
                        _token: _token,
                        id: id,
