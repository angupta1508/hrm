...
@section('content')
    <div class="card card-default">
        ...
        <div class="card-body">
            <form action="{{ route('admin.administration.shifts.store') }}" method="POST" enctype='multipart/form-data'>
                @csrf 
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="">
                            <input type="text" class="form-control" placeholder="{{ __('Shift Name') }}" name="shift_name"
                                id="shift_name" value="{{ old('shift_name') }}">
                            @error('shift_name')
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="">
                                <div class="flex-fill">
                                    {{ Form::select('shift_type', ['' => __('Select Shift Type')] + getListTranslate(config('constants.default_shift_type')), null, ['class' => 'form-select']) }}
                                </div>
                            @error('shift_type')
                                <p class="text-danger text-xs mt-2">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="">
                            <input type="time" class="form-control" placeholder="{{ __('From Time') }}" name="from_time"
                                id="from_time" value="{{ old('from_time') }}">
                            @error('from_time')
                                <p class="text-danger text-xs mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="">
                            <input type="time" class="form-control" placeholder="{{ __('To Time') }}" name="to_time"
                                id="to_time" value="{{ old('to_time') }}">
                            @error('to_time')
                                <p class="text-danger text-xs mt-2">{{ $message }}</p>
                            @enderror
                        </div>
