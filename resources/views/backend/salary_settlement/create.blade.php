...
@section('content')
    <div class="card card-default">
        ...
        <div class="card-body">
            <form action="{{ route('admin.payroll.salary-settlement.store') }}" method="POST" enctype='multipart/form-data'>
                @csrf
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <div class="">
                            <input type="month" class="form-control" placeholder="{{ __('Salary Settlement Name') }}"
                                name="settlement_month" id="settlement_month" value="{{ old('settlement_month') }}">
                    ...
                    <div class="col-md-4">
                        ...
                        <div>
                            {{ Form::select('type', ['name' => __('Select Payement Type')] + config('constants.payment_type'), old('type'), ['class' => 'form-select']) }}
                            @error('type')
                                <p class="text-danger text-xs mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="">
                            <textarea class="form-control" placeholder="{{ __('Description') }}" name="description" id="description">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="text-danger text-xs mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="d-flex justify-content-end mt-4">
                        <a href="{{ route('admin.payroll.salary-settlement.index') }}" type="button" name="button"
