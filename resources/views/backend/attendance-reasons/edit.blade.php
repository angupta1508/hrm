    ...
    <div class="card card-default">
        <div class="card-header pb-0">
            <div class="d-flex flex-row justify-content-between">
                <div>
                    <h5 class="mb-0">
                        {{ __('Edit New Attendance Reason') }}
                    </h5>
                </div>
            </div>
        </div> 
        <div class="card-body">
            <form action="{{ route('admin.attendence.attendance-reasons.update', $attendance_reason) }}" method="POST" enctype='multipart/form-data'>
                @method('PUT')
                @csrf
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="">
                            <input type="text" class="form-control" placeholder="{{ __('Attendance Reason Name') }}" name="name"
                                id="name" value="{{ old('name', $attendance_reason->name) }}">
                            @error('name')
