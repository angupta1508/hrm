        ...
        <div class="accordion-item mb-3">
            ...
            <div id="collapseFilter" class="accordion-collapse collapse @if (!empty($filter)) show @endif"
                ...
                <div class="accordion-body card-body p-3 text-sm opacity-8">
                    <form action="{{ route('admin.leave.holidays.index') }}" method="GET">
                        @csrf
                        <div class="border">
                            <div class="d-flex flex-row align-content-between flex-wrap">
                                <div class="p-2 flex-fill">
                                    <input type="text" id="holiday_name" name="holiday_name" class="form-control"
                                        value="{{ !empty($filter['holiday_name']) ? $filter['holiday_name'] : '' }}"
                                        autocomplete="off" placeholder="{{ __('Search By Holiday Name') }}">
                                </div>
                                <!-- <div class="p-2 flex-fill">
                                    <input type="text" id="holiday_type" name="holiday_type" class="form-control"
                                        value="{{ !empty($filter['holiday_type']) ? $filter['holiday_type'] : '' }}"
                                        autocomplete="off" placeholder="{{ __('Search By Holiday Type') }}">
                                </div> -->
                                <div class="p-2 flex-fill">
    ...
    <div class="card mb-4">
        <div class="card-header p-3">
            <div class="d-flex flex-row justify-content-between">
                ...
                <a href="{{ route('admin.leave.holidays.create') }}" class="btn bg-gradient-primary btn-sm mb-0">
        ...
        <div class="card-body p-3">
                ...
                <table class="table align-items-center mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                @sortablelink('holiday_name', __('Holiday Name'))
                            </th>
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                @sortablelink('date', __('Date'))
                            </th>
                        ...
                        @foreach ($holidays as $holiday)
                            <tr>
                                ...
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ wordlimit($holiday->holiday_name) }}
                                    </p>
                                </td>
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ wordlimit($holiday->date) }}
                                    </p>
                                </td>
                                ...
                                <td class="text-center">
                                    <div class="form-check form-switch d-inline-block">
                                        <input class="form-check-input updateStatus" type="checkbox" role="switch"
                                            {{ $holiday->status ? 'checked' : '' }} data-id="{{ $holiday->id }}">
                                    </div>
                                </td>

                                <td class="text-end">

                                    <form action="{{ route('admin.leave.holidays.destroy', $holiday->id) }}" method="Post">
                                        <a class="btn btn-warning btn-sm text-white" title="{{ __('Edit') }}"
                                            data-toggle="tooltip"
                                            href="{{ route('admin.leave.holidays.edit', $holiday->id) }}"><i
            ...
            {{ $holidays->appends($filter)->links('pagination::bootstrap-5') }}
        </div>
    </div>
@endsection
    ...
    <script>
        $(document).ready(function() {
            $('.updateStatus').on('change', function() {
                ...
                //
                $.ajax({
                    url: "{{ route('admin.leave.holidays.changeStatus') }}",
                    type: 'post',
                    data: {
                        _token: _token,
                        id: id,
