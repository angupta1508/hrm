        ...
        <div class="accordion-item mb-3"> 
            ...
            <div id="collapseFilter" class="accordion-collapse collapse @if (!empty($filter)) show @endif"
                ...
                <div class="accordion-body card-body p-3 text-sm opacity-8">
                    <form action="{{ route('admin.performance.performance-types.index') }}" method="GET">
                        @csrf
                        <div class="border">
    ...
    <div class="card mb-4">
        <div class="card-header p-3">
            <div class="d-flex flex-row justify-content-between">
                <div>
                    <h5 class="mb-0">{{ __('Performance Types') }}</h5>
                </div>
                <a href="{{ route('admin.performance.performance-types.create') }}" class="btn bg-gradient-primary btn-sm mb-0">
                    <i class="fa fa-plus" aria-hidden="true"></i>
                    {{ __('Add New Performance Type') }}
                </a> 
            </div>
        </div>
        <div class="card-body p-3">
                        ...
                        @foreach ($performance_types as $performance_type)
                            <tr>
                                ...
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ wordlimit($performance_type->name) }}
                                    </p>
                                </td>
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ $performance_type->created_at }}</p>
                                </td>
                                <td class="text-center">
                                    <div class="form-check form-switch d-inline-block">
                                        <input class="form-check-input updateStatus" type="checkbox" role="switch"
                                            {{ $performance_type->status ? 'checked' : '' }} data-id="{{ $performance_type->id }}">
                                    </div>
                                </td> 
                                <td class="text-end">
                                    <form action="{{ route('admin.performance.performance-types.destroy', $performance_type->id) }}" method="Post">
                                        <a class="btn btn-warning btn-sm text-white" title="{{ __('Edit') }}"
                                            data-toggle="tooltip" href="{{ route('admin.performance.performance-types.edit', $performance_type->id) }}">
            ...
            {{ $performance_types->appends($filter)->links('pagination::bootstrap-5') }}
        </div>
    </div>
@endsection
    ...
    <script>
        $(document).ready(function() {
            $('.updateStatus').on('change', function() {
                ...
                //
                $.ajax({
                    url: "{{ route('admin.performance.performance.changeStatus') }}",
                    type: 'post',
                    data: {
                        _token: _token,
                        id: id,
