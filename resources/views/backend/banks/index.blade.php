        ...
        <div class="accordion-item mb-3">
            ...
            <div id="collapseFilter" class="accordion-collapse collapse @if (!empty($filter)) show @endif"
                ...
                <div class="accordion-body card-body p-3 text-sm opacity-8">
                    <form action="{{ route('admin.administration.banks.index') }}" method="GET">
                        @csrf
                        <div class="border">
                            <div class="d-flex flex-row align-content-between flex-wrap">
                                <div class="p-2 flex-fill">
                                    <input type="text" id="search" name="search" class="form-control"
                                        ...
                                        placeholder="{{ __('Search By Bank Name') }}">
                                </div>
    ...
    <div class="card mb-4">
        <div class="card-header p-3">
            <div class="d-flex flex-row justify-content-between">
                <div>
                    <h5 class="mb-0">{{ __('Banks') }}</h5>
                </div>
                <a href="{{ route('admin.administration.banks.create') }}" class="btn bg-gradient-primary btn-sm mb-0">
        ...
        <div class="card-body p-3">
                ...
                <table class="table align-items-center mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                @sortablelink('bank_name', __('Bank Name'))
                            </th>
                        ...
                        @foreach ($banks as $bank)
                            <tr>
                                ...
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ wordlimit($bank->bank_name) }}
                                    </p>
                                </td>
                                <td class="text-center">
                                    <div class="avatar">
                                        <img class="w-100 border-radius-sm shadow-sm"
                                            src="{{ ImageShow(config('constants.bank_image_path'), $bank->bank_logo, 'small') }}">
                                    </div>
                                </td>
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ $bank->created_at }}</p>
                                </td>
                                <td class="text-center">
                                    <div class="form-check form-switch d-inline-block">
                                        <input class="form-check-input updateStatus" type="checkbox" role="switch"
                                            {{ $bank->status ? 'checked' : '' }} data-id="{{ $bank->id }}">
                                    </div>
                                </td>
                                <td class="text-end">
                                    <form action="{{ route('admin.administration.banks.destroy', $bank->id) }}" method="Post">
                                        <a class="btn btn-warning btn-sm text-white" title="{{ __('Edit') }}"
                                            data-toggle="tooltip" href="{{ route('admin.administration.banks.edit', $bank->id) }}">
            ...
            {{ $banks->appends($filter)->links('pagination::bootstrap-5') }}
        </div>
    </div>
@endsection
    ...
    <script>
        $(document).ready(function() {
            $('.updateStatus').on('change', function() {
                ...
                //
                $.ajax({
                    url: "{{ route('admin.administration.banks.changeStatus') }}",
                    type: 'post',
                    data: {
                        _token: _token,
                        id: id,
