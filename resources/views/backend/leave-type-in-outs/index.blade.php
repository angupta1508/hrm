        ...
        <div class="accordion-item mb-3">
            ...
            <div id="collapseFilter" class="accordion-collapse collapse @if (!empty($filter)) show @endif"
                ...
                <div class="accordion-body card-body p-3 text-sm opacity-8">
                    <form action="{{ route('admin.leave-type-in-outs.index') }}" method="GET">
                        @csrf
                        <div class="border">
    ...
    <div class="card mb-4">
        <div class="card-header p-3">
            <div class="d-flex flex-row justify-content-between">
                <div>
                    <h5 class="mb-0">{{ __('Leave Type In Out') }}</h5>
                </div>
                <a href="{{ route('admin.leave-type-in-outs.create') }}" class="btn bg-gradient-primary btn-sm mb-0">
                    <i class="fa fa-plus" aria-hidden="true"></i>
                    {{ __('Add New Leave Type In Out') }}
                </a>
            </div>
        </div>
        <div class="card-body p-3">
                ...
                <table class="table align-items-center mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                @sortablelink('name', __('leave type'))
                            </th>
                    ...
                    <tbody>
                        @foreach ($leave_type as $leave_type_in_out)
                            <tr>
                                ...
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">
                                        {{ wordlimit($leave_type_in_out->name) }}
                                    </p>
                                </td>
                                ...
                                <td class="text-center">
                                    <div class="form-check form-switch d-inline-block">
                                        <input class="form-check-input updateStatus" type="checkbox" role="switch"
                                            {{ $leave_type_in_out->status ? 'checked' : '' }}
                                            data-id="{{ $leave_type_in_out->id }}">
                                    </div>
                                </td>
                                <td class="text-end">
                                    <form action="{{ route('admin.leave-type-in-outs.destroy', $leave_type_in_out->id) }}"
                                        method="Post">
                                        <a class="btn btn-warning btn-sm text-white" title="{{ __('Edit') }}"
                                            data-toggle="tooltip"
                                            href="{{ route('admin.leave-type-in-outs.edit', $leave_type_in_out->id) }}">
            ...
            {{ $leave_type->appends($filter)->links('pagination::bootstrap-5') }}
        </div>
    </div>
@endsection
    ...
    <script>
        $(document).ready(function() {
            $('.updateStatus').on('change', function() {
                ...
                //
                $.ajax({
                    url: "{{ route('admin.leave-type-in-outs.changeStatus') }}",
                    type: 'post',
                    data: {
                        _token: _token,
                        id: id,
