        ...
        <div class="accordion-item mb-3">
            ...
            <div id="collapseFilter" class="accordion-collapse collapse @if (!empty($filter)) show @endif"
                ...
                <div class="accordion-body card-body p-3 text-sm opacity-8">
                    <form action="{{ route('admin.duty-types.index') }}" method="GET">
                        @csrf
                        <div class="border">
    ...
    <div class="card mb-4">
        <div class="card-header p-3">
            <div class="d-flex flex-row justify-content-between">
                <div>
                    <h5 class="mb-0">{{ __('Duty Types') }}</h5>
                </div>
                <a href="{{ route('admin.duty-types.create') }}" class="btn bg-gradient-primary btn-sm mb-0">
                    <i class="fa fa-plus" aria-hidden="true"></i>
                    {{ __('Add New Duty Type') }}
                </a>
            </div>
        </div>
        <div class="card-body p-3">
                ...
                <table class="table align-items-center mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                @sortablelink('duty_type', __('Duty Type'))
                            </th>
                        ...
                        @foreach ($duty_types as $duty_type)
                            <tr>
                                ...
                                <td class="text-capitalize">
                                    <p class="text-xs font-weight-bold mb-0">{{ wordlimit($duty_type->duty_type) }}
                                    </p>
                                </td>
                                ...
                                <td class="text-center">
                                    <div class="form-check form-switch d-inline-block">
                                        <input class="form-check-input updateStatus" type="checkbox" role="switch"
                                            {{ $duty_type->status ? 'checked' : '' }} data-id="{{ $duty_type->id }}">
                                    </div>
                                </td>
                                <td class="text-end">
                                    <form action="{{ route('admin.duty-types.destroy', $duty_type->id) }}" method="Post">
                                        <a class="btn btn-warning btn-sm text-white" title="{{ __('Edit') }}"
                                            data-toggle="tooltip" href="{{ route('admin.duty-types.edit', $duty_type->id) }}">
            ...
            {{ $duty_types->appends($filter)->links('pagination::bootstrap-5') }}
        </div>
    </div>
@endsection
    ...
    <script>
        $(document).ready(function() {
            $('.updateStatus').on('change', function() {
                ...
                //
                $.ajax({
                    url: "{{ route('admin.duty-type.changeStatus') }}",
                    type: 'post',
                    data: {
                        _token : _token,
                        id: id,
